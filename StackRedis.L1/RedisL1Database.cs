...
namespace StackRedis.L1
{
    public class RedisL1Database : IDatabase, IDisposable
    {
        ...
        /// <summary>
        /// Pauses the keyspace notification listener, so that notifications from Redis are not reflected in the memory cache.
        /// </summary>
        /// <param name="pause"></param>
        internal void PauseKeyspaceNotifications(bool pause = true)
        {
            if (_dbData.Listener != null)
            {
                _dbData.Listener.Paused = pause;
            }
        }
